...
const { getAll, create, getById, vote, deleteById, edit } = require('../services/posts');
...
router.get('/posts/:id', async (req, res) => {
    ...
    if (req.session.user) {
        ...
        if (userId === post.author._id.toString()) {
            ...
            post.isVoted = post.votes.map((v) => v._id.toString()).includes(userId);
        }
    }
...
router.get('/vote/:id/:value', isUser(), async (req, res) => {
    const postId = req.params.id;
    const post = postViewModel(await getById(postId));
    const userId = req.session.user._id;
    ...
    try {
        if (userId === post.author._id.toString()) {
            throw new Error('Author can not vote the this post');
        }
...
router.post('/edit/:id', isUser(), trimBody(), async (req, res) => {
    ...
    try {
        if (userId !== dbPost.author._id.toString()) {
            throw new Error('Only author can edit this post');
        }
